        ...
        for (x = 0; x != width; ++x) {
            for (i=-1 ; i<filterHeight-1 ; ++i) {
                for (j=-1 ; j<filterWidth-1 ; ++j) {
                    ...
                    sc_uint<24> source_rgb;
                    source_rgb.range(7,0) = R;
                    source_rgb.range(15,8) = G;
                    source_rgb.range(23,16) = B;
#ifndef NATIVE_SYSTEMC
					o_rgb.put(source_rgb);
#else
					o_rgb.write(source_rgb);
#endif
                }
            }
        }
...
void Testbench::output_data(){
	...
	for (y = 0; y != height; ++y) {
		for (x = 0; x != width; ++x) {
#ifndef NATIVE_SYSTEMC
			result_rgb = i_rgb.get();
#else
		    result_rgb = i_rgb.read();
#endif
